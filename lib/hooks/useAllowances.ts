...
import { AmountHumanReadable, TokenAmountHumanReadable, TokenAmountHumanReadableAndScaled, TokenBase } from '../types';
...
export function useAllowances(account: string | null, _tokens: (TokenBase | null)[], contract?: string) {
    const networkConfig = useNetworkConfig();
    contract = contract || networkConfig.balancer.vault;

    const tokens = _tokens.filter((token) => token !== null) as TokenBase[];
    const containsEth = tokens.filter((token) => token.address === networkConfig.eth.address).length > 0;
    const filteredTokens = tokens.filter((token) => token.address !== networkConfig.eth.address);
    ...
    const { data, ...rest } = useMultiCall({
        ...
        enabled: account !== null && tokens.length > 0,
        cacheTimeMs: ALLOWANCES_CACHE_TIME_MS,
    });

    const allowances: TokenAmountHumanReadableAndScaled[] = data
        ? filteredTokens.map((token, index) => {
              return {
                  address: token.address,
                  amount: data && data[index] ? formatFixed(data[index] || '0', token.decimals) : '0',
                  amountScaled: data && data[index] ? data[index] : '0',
              };
          })
        : [];
    ...
    if (containsEth) {
        allowances.push({
            ...
            amountScaled: MaxUint256.toString(),
        });
    }
